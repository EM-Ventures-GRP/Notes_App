...
def show_notes_grouped(notes: NotesDict) -> None:
    ...
    for cat, items in notes.items():
        ...
        show_numbered(items)
#---------------------------------------------------------------------------
def delete_note(notes: NotesDict, category: str, idx_one_based: int, seen: set[str]):
...
def edit_note(notes: NotesDict, seen: set[str]):
    ...
    #Duplicate  guard (anywher in the APP)
        ...
        if new_key in seen:
            print("Duplicate note detected! Edit canceled.")
            return False
...
def main():
    ...
    while True:
      ...
      if choice == "1":
            ...
            from_cat = input("Move FROM category: ").strip().lower()
            ...
            to_cat = (input("Move TO category (default: General): ").strip() or "General").lower()
